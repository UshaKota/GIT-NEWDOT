...
#draw a Correspondence analysis graph
png("correspondeAnalysis-digitools-access-Location.png", width = 1200, height = 800)

dev.acc.dig.tools.tab<-xtabs( ~tool.ser.dig + frq.ac.dig + LocationRespondent , data= dot.gen.acc.digi.dt)
kca<-CA(dev.acc.dig.tools.tab,graph = TRUE)
print(kca)
dev.off()
